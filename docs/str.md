...
```
\
The `echo` method allows you to easily `echo` the string. This method has optional `$prefix` and `$suffix` arguments.

```php
str($string)
->echo(suffix: '<br>')
->tolower
->echo(prefix: 'Lowercase: ');
```
\
...
```
\
There are several ways to instantiate a new instance of `WTFramework\Types\Str`.
